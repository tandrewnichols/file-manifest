 ...
 *  @param {Function|String} [callback] - An optional callback, which can also be "promise"
 *    to indicate that a proimse should be returned
 *
...
exports.generatePromised = function(dir, options) {
...
exports.standardizePath = function(dir) {
  if (dir.charAt(0) !== path.sep) {
    ...
    var caller = _.find(trace, function(callsite) {
      return callsite.getFileName().split('/').pop() !== 'file-manifest.js';
    });
...
exports.run = function(options) {
  var reduce = function(memo, file, next) {
    ...
    return (options.reduce || exports.reduce).call(options, memo, fileObj, next);
  };
...
exports.reduce = function(manifest, file, next) {
